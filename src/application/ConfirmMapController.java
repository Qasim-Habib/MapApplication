...
public class ConfirmMapController {
	...
	@FXML
	private TableView<Map> searchTable;
	...
	@FXML
	private TableColumn<Map, String> UpdateCol;
	...
	void initialize() throws IOException, Exception {
		...
		searchTable.setRowFactory(tv -> {
			...
			row.setOnMouseClicked(event -> {
				if (event.getClickCount() == 1 && (!row.isEmpty())) {
					Map MapRow = searchTable.getSelectionModel().getSelectedItem();
					Globals.map = (Map) MapRow;
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn.setOnAction(event -> {
								...
								Globals.backLink = "ConfirmMap.fxml";
								AnchorPane pane;
								try {
									pane = FXMLLoader.load(url);
									Scene scene = new Scene(pane);
		...
		flMap = new FilteredList<Map>(dataMap, p -> true);// Pass the data to a filtered list
		searchTable.setItems(flMap);// Set the table's items using the filtered list
